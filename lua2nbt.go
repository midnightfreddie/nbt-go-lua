...
func writePayload(w io.Writer, m map[string]interface{}, tagType float64) error {
	...
	case 1:
		if i, ok := m["value"].(float64); ok {
			...
			return LuaNbtError{fmt.Sprintf("Tag 1 Byte value field '%v' not an integer", m["value"]), err}
		}
	case 2:
		if i, ok := m["value"].(float64); ok {
			...
			return LuaNbtError{fmt.Sprintf("Tag 2 Short value field '%v' not an integer", m["value"]), err}
		}
	case 3:
		if i, ok := m["value"].(float64); ok {
			...
			return LuaNbtError{fmt.Sprintf("Tag 3 Int value field '%v' not an integer", m["value"]), err}
		}
		/*
			case 4:
				if int64Map, ok := m["value"].(map[string]interface{}); ok {
					...
					return LuaNbtError{fmt.Sprintf("Tag 4 Long value field '%v' not an object", m["value"]), err}
				}
			case 5:
				if f, ok := m["value"].(float64); ok {
					...
					// return LuaNbtError{fmt.Sprintf("Tag 5 Float value field '%v' not a number", m["value"]), err}
					f = math.NaN()
					err = binary.Write(w, byteOrder, float32(f))
					if err != nil {
			...
			case 6:
				if f, ok := m["value"].(float64); ok {
					...
					// return LuaNbtError{fmt.Sprintf("Tag 6 Double value field '%v' not a number", m["value"]), err}
					f = math.NaN()
					err = binary.Write(w, byteOrder, f)
					if err != nil {
						return LuaNbtError{"Error writing float64 payload", err}
				...
				if values, ok := m["value"].([]interface{}); ok {
					...
					for _, value := range values {
						if i, ok := value.(float64); ok {
							...
							return LuaNbtError{fmt.Sprintf("Tag 7 Byte Array element value field '%v' not an integer", m["value"]), err}
						}
					}
				} else {
					return LuaNbtError{fmt.Sprintf("Tag 7 Byte Array element value field '%v' not an array", m["value"]), err}
				}
			case 8:
				if s, ok := m["value"].(string); ok {
					...
					return LuaNbtError{fmt.Sprintf("Tag 8 String value field '%v' not a string", m["value"]), err}
				}
			case 9:
				...
				if listMap, ok := m["value"].(map[string]interface{}); ok {
					...
					return LuaNbtError{fmt.Sprintf("Tag 9 List value field '%v' not an object", m["value"]), err}
				}
			case 10:
				if values, ok := m["value"].([]interface{}); ok {
					for _, value := range values {
					...
					return LuaNbtError{fmt.Sprintf("Tag 10 Compound value field '%v' not an array", m["value"]), err}
				}
			case 11:
				if values, ok := m["value"].([]interface{}); ok {
					err = binary.Write(w, byteOrder, int32(len(values)))
					...
					return LuaNbtError{fmt.Sprintf("Tag Int Array value field '%v' not an array", m["value"]), err}
				}
			case 12:
				if values, ok := m["value"].([]interface{}); ok {
					...
					return LuaNbtError{fmt.Sprintf("Tag 12 Long Array element value field '%v' not an array", m["value"]), err}
				}
			default:
